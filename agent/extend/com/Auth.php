...
class Auth{
    ...
    public function getGroups($uid) {
        ...
        if (isset($groups[$uid]))
            return $groups[$uid];
        $user_groups = \think\Db::table('cms_auth_group_access')
            ->alias('a')
            ->join("cms_auth_group g", "g.id=a.group_id")
    ...
    protected function getAuthList($uid,$type) {
        ...
        //读取用户组所有权限规则
        $rules = \think\Db::table('cms_auth_rule')->where($map)->field('condition,name')->select();
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userinfo[$uid])){
             $userinfo[$uid]=\think\Db::table('cms_admin')->where('id',$uid)->find();
        }
        return $userinfo[$uid];
    }
